...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S" and len(self.buffer) > 0:
            ...
            last = self.stack.pop()
            secondLast = self.stack.pop()
            self.stack.append(last)
            self.dependencies.append((last, secondLast))
        elif transition == "RA" and len(self.stack) > 1:
            last = self.stack.pop()
            secondLast = self.stack[-1]
            self.dependencies.append((secondLast, last))
        ### END YOUR CODE
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        batch = unfinished_parses[:batch_len]
        transitions = model.predict(batch)
        for i in range(batch_len-1, -1 ,-1):
